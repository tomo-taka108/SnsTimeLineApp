...
| 使用API | #7 投稿取得, #10 コメント一覧, #11 コメント投稿, #12 コメント編集, #13 コメント削除, #14/#15 いいね |
...
| `[⋯]`メニューの項目 | 「✏️ 編集」「🗑 削除」の2つ（自分のコメントのみ表示） |
...
**コメント編集（F-CM-03、[09_decision_log.md](09_decision_log.md) D-51 によりMVPへ前倒し）**

> **モーダル（MD-02のような専用画面）は使わない。** コメントは投稿より分量が小さく、モーダルを開く操作は過剰なため、**インライン編集**を採用する。専用モーダル（MD-04相当）は設けない。

| 項目 | 仕様 |
|---|---|
| 開始 | `[⋯]` → 「✏️ 編集」で、そのコメントの本文表示がその場で編集フォーム（textarea）に切り替わる |
| 初期値 | 現在の本文をそのままフォームに読み込む |
| バリデーション | コメント投稿と同じ。必須、1〜280文字。文字数カウンタを表示 |
| 保存後 | フォームを閉じて本文表示に戻り、**そのコメントに「編集済み」を表示**。コメント数は変わらない |
| キャンセル | 編集前の本文表示に戻る（入力内容は破棄） |
| 失敗時 | フォームを閉じず、入力内容を保持したままトーストでエラー表示 |
...
- 変更成功時はトースト表示のみ。**Phase2の実装時は、パスワード変更に合わせてリフレッシュトークンを全失効させること**（[05_api_design.md](05_api_design.md) #4）。発行済みのアクセストークンは最大15分間有効なまま残る。
...
| 認証切れ（401、それ以外） | **まず `POST /auth/refresh` で再発行を試み**、成功したら元の操作を続行（ユーザーには見えない）。失敗したら両トークンを破棄し SC-01 へ自動遷移（F-CO-02）。トーストで「セッションの有効期限が切れました」 |
