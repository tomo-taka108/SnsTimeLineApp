...
| 複数端末のセッション管理（端末ごとの表示・個別ログアウト） | 「どの端末からログイン中か」を一覧・選択して切る機能は作らない。**ただしトークンの失効管理は行う**（ログアウトでそのユーザーの全リフレッシュトークンを失効させる）。当初の「失効管理は行わない」という記述は[09_decision_log.md](09_decision_log.md) D-29で見直した |
...
- ステートレス認証（JWT）のため、サーバー側でセッションを保持しない。**ただしリフレッシュトークンだけはDBで管理する**（[09_decision_log.md](09_decision_log.md) D-29）。ログアウトはリフレッシュトークンの失効＋クライアント側のトークン破棄で実現する。**発行済みのアクセストークンは失効できず、最大15分間有効なまま**である点は制約として残る。
