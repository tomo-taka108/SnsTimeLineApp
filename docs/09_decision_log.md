...
## D-49 ユーザー検索の第2段階は「部分一致」に限定し、タイプミス補正は入れない

| 項目 | 内容 |
|---|---|
| **日付** | 2026-08-30 |
| **論点** | [04_data_model.md](04_data_model.md) 6.3 の第2段階は `similarity()` による純粋なあいまい一致であり、「たろう」で検索したときに「田中たろう」（中間一致）だけでなく「たろお」（タイプミス）もヒットする。この挙動をそのまま採用するか |
| **選択肢** | A. 設計書どおり（閾値0.3のあいまい一致） / **B. 部分一致に限定する** / C. 閾値を0.5程度に上げて調整する |
| **決定** | **B**（Issue #21） |
| **状態** | 決定済み |

**理由**

**人名・アカウント検索でタイプミス補正を入れないのが実務の主流である。** X / Slack / GitHub 等のユーザー検索・メンション補完は、いずれも部分一致ベースで fuzzy matching を持たない。理由は3つある。

| # | 理由 |
|---|---|
| 1 | ユーザー名は「知っている文字列」を打ち込む対象であり、うろ覚えの単語を投げる全文検索とは性質が違う |
| 2 | フォロー・メンションは相手に通知が飛ぶ操作のため、似た名前の他人が上位に出る誤ヒットの実害が大きい |
| 3 | 検索語が2〜3文字でも候補が十分絞れるため、あいまい一致で「拾いに行く」必要がない |

fuzzy を採用しているサービスも、「正確な一致を結果に出し、補正候補は別枠（もしかして: ◯◯）」とするのが定石であり、**両者を1つのリストに混ぜない**。本アプリには「もしかして」の枠を作る要件がないため、混ぜない側に倒す。

**pg_trgm を使う学習目的は損なわれない。** GINインデックスによる中間一致の高速化（`%` 演算子）と `similarity()` による関連度ソートは引き続き使う。捨てるのは「実際には文字を含まないものまで結果に入れる」部分だけである。

**実装**: 絞り込みは `LIKE '%q%'` のみで行う。`similarity()` は `ORDER BY` にのみ置く。

```sql
WHERE deleted_at IS NULL
  AND id <> :me
  AND (lower(username) LIKE '%' || lower(:q) || '%' ESCAPE '\'
    OR lower(display_name) LIKE '%' || lower(:q) || '%' ESCAPE '\')   -- 含むことを保証
ORDER BY
  CASE WHEN lower(username) LIKE lower(:q) || '%' ESCAPE '\' THEN 0    -- 前方一致を最優先
       WHEN lower(display_name) LIKE lower(:q) || '%' ESCAPE '\' THEN 1
       ELSE 2 END,
  GREATEST(similarity(username, :qRaw), similarity(display_name, :qRaw)) DESC,
  id                                                                    -- タイブレーカー必須
```

**実装中の修正（重要）**: 当初は「`%` 演算子でGINを使って候補を絞り、`LIKE` で確定させる」形にしていたが、**実機検証で誤りが判明したため `%` による絞り込みを外した。**

`%` 演算子は「類似度が閾値以上か」を見るものであり、**「部分文字列を含むか」の上位集合ではない**。実データでは次のようになる。

| `display_name` | `q` | `LIKE '%q%'` | `similarity()` | `%` 演算子（閾値0.3） |
|---|---|---|---|---|
| `田中たろう` | `たろう` | **true**（出すべき） | 0.25 | **false**（落ちる） |
| `たろお` | `たろう` | false | 0.33 | true |

つまり `%` を `AND` で重ねると、**本来ヒットすべき部分一致（田中たろう）が落ち、タイプミス（たろお）だけが通る**という逆の結果になる。特に日本語は3文字の検索語だとトライグラムが少なく、前後に文字が付くだけで類似度が急落する。`word_similarity`（`<%`、閾値0.6）でも同じく 0.5 で落ちた。閾値を下げれば拾えるが、今度はタイプミスまで通り本決定の意図に反する。**両立しないため `%` での絞り込みは行わない。**

**その結果、この検索は `Seq Scan` になる**（中間一致にB-treeは効かない、[04_data_model.md](04_data_model.md) 6.4）。50,000件を投入した `EXPLAIN ANALYZE` で確認済み。
...
## D-51 コメント編集（F-CM-03）をMVPへ前倒しする

| 項目 | 内容 |
|---|---|
| **日付** | 2026-08-30 |
| **論点** | [D-10](#d-10-優先度の切り分け方針削除はmvp編集はphase2) ではコメント編集をPhase2としたが、投稿編集（F-PO-04）が [D-30](#d-30-投稿編集f-po-04をmvpへ前倒しする) で前倒し済みの状態で、コメント編集も実装するよう依頼があった |
| **選択肢** | A. D-10どおりPhase2のまま見送る / **B. MVPへ前倒しする** |
| **決定** | **B** |
| **状態** | 決定済み |
...
- **利用者からの明示的な要望。** [D-30](#d-30-投稿編集f-po-04をmvpへ前倒しする) と同じ理由付け。
- **D-10で「DBスキーマとAPI設計には最初から編集を織り込む」と決めていたため、前倒しのコストが低い。** `comments.edited_at` カラムと `PATCH /comments/{commentId}`（API #12）は設計時点で確保済みで、マイグレーションの追加が不要だった。
- 削除に比べて編集は取り消しが効く操作であり、実装リスクは小さい（D-30と同様）。
- **編集UIはモーダル（MD-02方式）ではなくインライン編集を採用した。** コメントは1件あたりの分量が投稿より小さく、モーダルを開く操作が過剰なため。既存の `CommentForm`（コメント投稿欄）を初期値・ラベル・キャンセルボタン対応にして流用し、新規モーダル（MD-04相当）は設けない。

**comment_count は変更しない。** 編集はカウンタに影響しない（[D-01](#d-01-いいね数コメント数のカウント方式)の対象外）。

**D-10は撤回しない。** 「削除はMVP、編集はPhase2」という判断の経緯自体は学習の記録として残す。本エントリは、その後の状況変化（利用者の要望）による範囲変更として追記する。

**影響範囲**: [02_feature_list.md](02_feature_list.md) F-CM-03 / [03_screen_design.md](03_screen_design.md) SC-04 / [05_api_design.md](05_api_design.md) #12 / [07_architecture.md](07_architecture.md) 7.1
...
## D-59 一覧SQLと件数SQLは、論理削除の絞り込みを必ず揃える

| 項目 | 内容 |
|---|---|
| **日付** | 2026-09-09 |
| **論点** | 一覧を返すSQL（`findTimeline` / `findFollowing` / `findFollowers`）と件数を数えるSQL（`countNewer` / `countFollowing` / `countFollowers`）で、退会ユーザーの除外が揃っていなかった（Issue #44）。件数側にもJOINを足すか、性能を優先して現状を許容するか |
| **選択肢** | **A. count側にも同じJOINを足して揃える** / B. 性能を優先し不一致を許容する / C. 非正規化カウンタを導入する |
| **決定** | **A** |
| **状態** | 決定済み |

**理由**

- **不一致はそのまま利用者に見える。** 「フォロー中 1」と表示されるのに一覧を開くと空、「新着1件」のバナーを押しても何も出てこない、という形で現れる。**エラーにならず静かに壊れる**ため、利用者は自分の操作を疑うことになる。
- **B（性能優先）を採るほどの負荷差が無い。** `follows` 側のインデックス（`idx_follows_follower` / `idx_follows_followee`）で先に絞り込んでから `users` を**主キーでJOIN**するため、走査対象は一覧取得と変わらない。「一覧は出せるが数えられない」という状況は考えにくい。
- **C（非正規化カウンタ）は [D-36](#d-36-プロフィールのカウント投稿数フォロー中数フォロワー数は非正規化しない) の判断と矛盾する。** フォロー数は都度 `COUNT(*)` する方針を既に採っており、ここだけ覆す理由が無い。いいね数・コメント数を非正規化しているのは更新頻度と表示頻度が桁違いだからで（[D-01](#d-01-いいね数コメント数のカウント方式)）、フォロー数は当てはまらない。
- **「一覧に出ないものは数えない」を規則として明文化する。** 個別の修正ではなく、今後SQLを足すときの判断基準にする。

**あわせて修正したもの**

| # | 箇所 | 内容 |
|---|---|---|
| ① | `PostMapper.countNewer` | `findTimeline` と同じ `JOIN users ... AND u.deleted_at IS NULL` を追加 |
| ② | `FollowMapper.countFollowers` / `countFollowing` | 同上（`findFollowers` / `findFollowing` と揃える） |
| ③ | `CommentMapper.findByPostId` | 親投稿の `deleted_at` を見ていなかった。`JOIN posts` を追加し、**SQL単体でも「削除済み投稿のコメントを返さない」ことを保証**する。Service が先に404を返すため実害は無かったが、その順序に依存していた（[D-60](#d-60-一覧apiはバリデーションを存在チェックより先に行う)で順序を変更したため、依存を残さない） |

> **③はテストの価値が出た例。** 実害の無い潜在的な不整合だったが、[D-60](#d-60-一覧apiはバリデーションを存在チェックより先に行う)で処理順を変えた際に顕在化しうる位置にあった。Mapper層のテストが「SQL単体では防げていない」ことを記録していたため、順序変更と同時に安全側へ倒せた。

**代償**: 件数SQLに毎回JOINが1つ増える。ただし上記のとおり走査対象は変わらない。

**影響範囲**: `backend/src/main/resources/mapper/PostMapper.xml` / `FollowMapper.xml` / `CommentMapper.xml` / 対応するMapper層のテスト（`PostMapperTest` #230・#230b、`FollowMapperTest` #246・#247・#247b、`CommentMapperTest` #239・#239b）

---

## D-60 一覧APIは、バリデーションを存在チェックより先に行う

| 項目 | 内容 |
|---|---|
| **日付** | 2026-09-09 |
| **論点** | 「存在しないID＋不正な `limit`」という同じ状況に対し、`PostService` は400、`CommentService` / `FollowService` は404を返していた（Issue #37）。どちらに統一するか |
| **選択肢** | **A. バリデーション（400）を先に統一** / B. 存在チェック（404）を先に統一 / C. 現状のまま許容する |
| **決定** | **A** |
| **状態** | 決定済み |

**理由**

- **リクエスト自体が不正なら、リソースを探す前に弾くのが自然。** `limit=999` は「何を探すか」以前の誤りであり、対象の存在有無に関係なく成立しない。
- **無駄なDBアクセスが減る。** 不正なリクエストでは `findById` まで到達しない。テストでも `verify(..., never()).findById(anyLong())` で到達しないことを固定した。
- **[D-14](#d-14-管理者ロールを設けるか)と矛盾しない。** D-14 が定めているのは**認可の**順序（存在→404、所有者→403）であり、その目的は「存在しないリソースに対して403を返すと存在有無が漏れる」ことの防止にある。400はリクエスト形式の誤りで、対象を特定する前段の話。**認可の判断に入る前に弾いている**ため、D-14の情報漏洩対策を損なわない。
- **C（現状維持）を採らない理由。** 実害は小さいが、**同じ状況で結果が違うことは、APIを使う側が「なぜここだけ違うのか」を毎回調べる必要がある**という形のコストになる。統一のコストが3箇所の行の入れ替えで済むうちに揃える。

**この判断の対象外**

- **`PostService.getUserPosts` はユーザーの存在チェック自体を行っていない**（存在しないユーザーIDでも空配列で200を返す）。これは「順序」ではなく「有無」の論点であり、本判断の範囲外として現状を維持する。変更すると `GET /users/{id}/posts` のレスポンスが 200→404 に変わり、フロントの分岐にも影響するため。

**影響範囲**: `CommentService.getComments` / `FollowService.getFollowing` / `getFollowers` / 対応するテスト（`CommentServiceTest` #98、`FollowServiceTest` #112）

---

## D-61 クライアント側の画像検証を共通関数に集約する

| 項目 | 内容 |
|---|---|
| **日付** | 2026-09-09 |
| **論点** | 画像の形式・サイズ検証が3箇所（投稿作成・アバター・カバー）にインラインで重複していた（Issue #47）。共通化するか、D-38 の「重複を許容する基準」に照らして現状を維持するか |
| **選択肢** | **A. `validateImageFile` を `api/files.ts` に切り出す** / B. 重複を許容する |
| **決定** | **A** |
| **状態** | 決定済み |

**理由**

- **D-38 の許容基準を超えている。** 同基準は「2箇所まで、かつ変更理由が異なるなら許容」としているが、ここは**3箇所**あり、**変更理由も完全に同一**（許可形式やサイズ上限が変わればすべて同時に変わる）。
- **エラーメッセージも重複していた。** 文言だけ1箇所直すと、同じ操作なのに画面によって表示が変わる。
- **テストの置き場ができる。** インラインのままではコンポーネントを描画しないと検証できないが、切り出したことで純粋関数として境界値を直接確認できる（5MBちょうど／+1バイト）。実際にミューテーション2件（境界の `>` → `>=`、形式チェックの無効化）で赤くなることを確認した。
- **サーバー側の検証は変更しない。** クライアント側は迂回できるため、これは利便性のための事前確認であって検証ではない。実際の検証はマジックバイトで行う（[D-42](#d-42-アップロードサイズ超過は-maxuploadsizeexceededexception-を捕捉して413にする)）。この位置づけは関数のコメントに残した。

**影響範囲**: `frontend/src/api/files.ts`（`validateImageFile` を追加）/ `PostComposer.tsx` / `ProfileEditPage.tsx`（アバター・カバーの2箇所）/ `frontend/src/api/files.test.ts`（#396b〜#396g）
