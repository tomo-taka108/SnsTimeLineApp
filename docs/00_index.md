...
ステップ5（いいね）、ステップ6（コメント）、ステップ10（削除・論理削除）、
ステップ11（無限スクロール）が完了**。ステップ8（フォロー）はタイムラインのタブとSQLのみ先行完了。
次はステップ7（画像アップロード）。
実装で新たに決めた事項は [09_decision_log.md](09_decision_log.md) D-25〜D-35 に記録している
（MyBatis採用 / jjwt採用 / パスワードのトリム除外 / JDK 25 + Spring Boot 4.1.0 / リフレッシュトークン方式 /
投稿編集のMVP前倒し / 新着通知バナー / カーソル精度の修正 / いいねの冪等性 / 重複ロジックの許容基準 など）。
...
> 編集はPhase2）。**画像添付は未実装**（[07_architecture.md](07_architecture.md) 7.1）。
