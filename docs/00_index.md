...
**現在の進捗**: ステップ1（`V1` / `V2`）と**ステップ2（認証）が完了**。
さらに**ステップ4のうち認証画面（SC-01 / SC-02 / SC-12）が先行して完了**している。次はステップ3。
実装で新たに決めた事項は [09_decision_log.md](09_decision_log.md) D-25〜D-29 に記録している
（MyBatis採用 / jjwt採用 / パスワードのトリム除外 / JDK 25 + Spring Boot 4.1.0 / リフレッシュトークン方式）。

```
1. DB構築（Flywayマイグレーション）        ← V1・V2 完了
2. 認証（#1〜#3, #27, #28）                ← 完了
3. 投稿作成・全体TL（#5, #6, #7）          ← 次はここ
4. フロント: ログイン + タイムライン表示    ← ログイン側は完了。TL表示は3の後
...
> **`/`（SC-03）は仮ページ。** 認証の動作確認用にログイン中ユーザーを表示するだけで、
> 投稿一覧は未実装（[07_architecture.md](07_architecture.md) 7.1）。
