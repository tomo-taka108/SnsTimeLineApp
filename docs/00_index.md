...
**現在の進捗**: ステップ1（`V1`〜`V6`）、**ステップ2（認証）、ステップ3（投稿）、ステップ4（フロント）、
ステップ5（いいね）、ステップ6（コメント）、ステップ7（画像アップロード・配信）、ステップ8（フォロー）、
ステップ9（プロフィール）、ステップ10（削除・論理削除）、ステップ11（無限スクロール）が完了**。
次は画像の連携（画像付き投稿 F-PO-02・プロフィール画像変更 F-US-04）。
実装で新たに決めた事項は [09_decision_log.md](09_decision_log.md) D-25〜D-42 に記録している
...
`@MapperScan` の限定 / アップロードサイズ超過の413化 など）。

```
1. DB構築（Flywayマイグレーション）        ← V1〜V6 完了
2. 認証（#1〜#3, #27, #28）                ← 完了
