...
**現在の進捗**: ステップ1（`V1`〜`V7`）、**ステップ2〜11がすべて完了**（[02_feature_list.md](02_feature_list.md)
6章のMVP完了判定チェックリストが機能単位ではすべて埋まった）。画像アップロード・配信に続き、
投稿への画像添付（F-PO-02）とプロフィール画像（F-US-04、削除も含む）を実装したことで、
画像機能一式が使える状態になった。
実装で新たに決めた事項は [09_decision_log.md](09_decision_log.md) D-25〜D-46 に記録している
（MyBatis採用 / jjwt採用 / パスワードのトリム除外 / JDK 25 + Spring Boot 4.1.0 / リフレッシュトークン方式 /
投稿編集のMVP前倒し / 新着通知バナー / カーソル精度の修正 / いいねの冪等性 / 重複ロジックの許容基準 /
プロフィールのカウント方式 / フォローの冪等性 / フォロー一覧のMVP前倒し / S3とLOCALの二本立て /
`@MapperScan` の限定 / アップロードサイズ超過の413化 / ファイル所有者チェックの集約 /
画像を渡す引数の必須化 / ライトボックスの独立実装 など）。

```
1. DB構築（Flywayマイグレーション）        ← V1〜V7 完了
2. 認証（#1〜#3, #27, #28）                ← 完了
3. 投稿作成・全体TL（#5, #6, #7, #8, #9, #29）← 完了
4. フロント: ログイン + タイムライン表示    ← 完了
5. いいね（#14, #15）← カウンタの整合性を作り込む     ← 完了
6. コメント（#10, #11, #12, #13）          ← 完了（編集(#12)もD-51でMVPへ前倒し）
7. 画像アップロード（#25, #26）            ← 完了。投稿への添付は次のPR
8. フォロー + フォロー中TL（#21, #22）← MVPの一周が完成  ← 完了
9. プロフィール（#17〜#19、#23、#24）      ← 完了
10. 削除と論理削除の徹底                    ← 完了
11. 無限スクロール                          ← 完了
```

> **`/`（SC-03）はタイムラインとして実装済み。** 無限スクロール・投稿作成（モーダル）・編集・削除・
> 新着通知バナー・いいねを含む。**投稿詳細（SC-04）にコメント欄も実装済み**（投稿・表示・
> 編集・削除）。**プロフィール（SC-05）・プロフィール編集（SC-06）・フォロー中一覧（SC-08）・
> フォロワー一覧（SC-09）も実装済み。** フォロー中タブは実際にフォロー中ユーザーの投稿を表示する。
> **画像添付・プロフィール画像は未実装**（[07_architecture.md](07_architecture.md) 7.1）。
