...
**現在の進捗**: ステップ1（`V1`〜`V4`）、**ステップ2（認証）、ステップ3（投稿）、ステップ4（フロント）、
ステップ11（無限スクロール）が完了**。ステップ8（フォロー）はタイムラインのタブとSQLのみ先行完了。
次はステップ5（いいね）。
実装で新たに決めた事項は [09_decision_log.md](09_decision_log.md) D-25〜D-33 に記録している
（MyBatis採用 / jjwt採用 / パスワードのトリム除外 / JDK 25 + Spring Boot 4.1.0 / リフレッシュトークン方式 /
投稿編集のMVP前倒し / 新着通知バナー / カーソル精度の修正 など）。

```
1. DB構築（Flywayマイグレーション）        ← V1〜V4 完了
2. 認証（#1〜#3, #27, #28）                ← 完了
3. 投稿作成・全体TL（#5, #6, #7, #8, #9, #29）← 完了
4. フロント: ログイン + タイムライン表示    ← 完了
5. いいね（#14, #15）← カウンタの整合性を作り込む     ← 次はここ。likesテーブルはV3で作成済み
6. コメント（#10, #11, #13）
7. 画像アップロード（#25, #26）
8. フォロー + フォロー中TL（#21, #22）← MVPの一周が完成  ← タブ・SQLのみ先行完了
9. プロフィール（#17〜#19）
10. 削除と論理削除の徹底                    ← 投稿削除のみ完了
11. 無限スクロール                          ← 完了
```

> **`/`（SC-03）はタイムラインとして実装済み。** 無限スクロール・投稿作成（モーダル）・編集・削除・
> 新着通知バナーを含む。**いいねボタンは表示のみ**（次のステップで実装）、
> **コメント欄・画像添付は未実装**（[07_architecture.md](07_architecture.md) 7.1）。
