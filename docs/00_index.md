...
**現在の進捗**: ステップ1（`V1`〜`V5`）、**ステップ2（認証）、ステップ3（投稿）、ステップ4（フロント）、
ステップ5（いいね）、ステップ6（コメント）、ステップ8（フォロー）、ステップ9（プロフィール）、
ステップ10（削除・論理削除）、ステップ11（無限スクロール）が完了**。
次はステップ7（画像アップロード）と、それに続くプロフィール画像変更（F-US-04）・画像付き投稿（F-PO-02）。
実装で新たに決めた事項は [09_decision_log.md](09_decision_log.md) D-25〜D-39 に記録している
（MyBatis採用 / jjwt採用 / パスワードのトリム除外 / JDK 25 + Spring Boot 4.1.0 / リフレッシュトークン方式 /
投稿編集のMVP前倒し / 新着通知バナー / カーソル精度の修正 / いいねの冪等性 / 重複ロジックの許容基準 /
プロフィールのカウント方式 / フォローの冪等性 / フォロー一覧のMVP前倒し など）。

```
1. DB構築（Flywayマイグレーション）        ← V1〜V5 完了
2. 認証（#1〜#3, #27, #28）                ← 完了
3. 投稿作成・全体TL（#5, #6, #7, #8, #9, #29）← 完了
4. フロント: ログイン + タイムライン表示    ← 完了
5. いいね（#14, #15）← カウンタの整合性を作り込む     ← 完了
6. コメント（#10, #11, #13）               ← 完了。編集（#12）はPhase2
7. 画像アップロード（#25, #26）            ← 次はここ
