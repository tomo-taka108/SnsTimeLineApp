...
**タイムライン実装で今回のスコープに含めなかったもの**（投稿カードの見た目は用意しているが動かない）:

| 項目 | 状態 |
|---|---|
| いいね（F-LK-01〜03） | `likes` テーブルは作成済み。カードのボタンは**表示のみ**で `onClick` を持たない |
| コメント（F-CM-01〜04） | 未着手。`commentCount` は常に0 |
...
| # | 内容 | 完了の目安 | 状態 |
|---|---|---|---|
| 1 | DB構築（Flywayマイグレーション） | 全テーブルと制約・インデックスが作られる | **`V1`(users) / `V2`(refresh_tokens) / `V3`(posts, likes) / `V4`(follows) 完了** |
...
| 5 | いいね（#14, #15） | **カウンタの整合性をここで作り込む。二重いいねのテストを必ず書く** | 未着手。**`likes` テーブルは V3 で作成済み**。投稿カードのいいねボタンは表示のみ |
...
| 9 | プロフィール（#17〜#19） | | 未着手 |
| 10 | 削除（#9, #13）と論理削除の徹底 | 削除済みがどこにも出ないことを確認 | **投稿削除（#9）は完了**。コメント削除（#13）はコメント機能とあわせて未着手 |
| 11 | 無限スクロール（カーソルページネーション） | **シードデータで25件以上の投稿を用意して確認** | **完了。カーソルの時刻精度を秒からマイクロ秒に変更した（D-33）** |

> **自動テストは今回のスコープ外**（別タイミングで実装予定）。ステップ5の「二重いいねのテスト」もそのときに書く。
