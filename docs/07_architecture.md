...
| パスワード変更（F-AU-05, SC-11, #4） | Phase2のため未実装 |

> **コメント編集（F-CM-03, #12）は実装済み**（[09_decision_log.md](09_decision_log.md) D-51 によりMVPへ前倒し）。投稿・表示・編集・削除（F-CM-01〜04）すべて実装済み。

> **ユーザー検索（F-US-05, SC-07, #20）は実装済み**（Issue #21）。本アプリで唯一のオフセットページネーションであり、タイプミス補正は入れない（[09_decision_log.md](09_decision_log.md) D-49）。
...
| 6 | コメント（#10, #11, #12, #13） | カウンタの非対称ルールを実装 | **完了**。コメント編集（#12）も[09_decision_log.md](09_decision_log.md) D-51 によりMVPへ前倒し実装済み |
