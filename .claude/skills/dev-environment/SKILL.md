...
> タイムライン（全体／フォロー中、無限スクロール）、コメント（投稿・表示・編集・削除）、いいね、
> プロフィール表示・編集、フォロー・フォロー中一覧・フォロワー一覧。
> 画像添付・プロフィール画像・ユーザー検索も実装済み。
> 未実装はいいねしたユーザー一覧（SC-10）・パスワード変更（SC-11）
> （[docs/07_architecture.md](../../../docs/07_architecture.md) 7.1）。
