...
- [ ] **実装フェーズ** — 進行中
  - [x] DB構築（`V1__create_users.sql` / `V2__create_refresh_tokens.sql` / `V3__create_posts_and_likes.sql` / `V4__create_follows.sql` / `V5__create_comments.sql`）
  - [x] **認証・認可のバックエンド** — 新規登録 / ログイン / `GET /auth/me` / **トークン再発行 / ログアウト**（アクセストークン＋リフレッシュトークン方式）
  - [x] **認証のフロントエンド** — 新規登録 / ログイン / ログアウト画面、ルートガード、401時のトークン自動再発行
  - [x] **投稿作成・全体タイムライン** — 作成 / 詳細 / 編集 / 削除、無限スクロール、新着通知バナー、いいね
  - [x] **コメント** — 投稿 / 表示 / 削除（編集はPhase2）
  - [x] **フォロー・フォロー中タイムライン** — フォロー / 解除（冪等）、フォロー中一覧、フォロワー一覧
  - [x] **プロフィール** — 表示（自分／他人）、編集（表示名・自己紹介・**プロフィール画像**）
  - [x] **画像投稿** — アップロード / 配信 / バリデーション（形式・サイズ・マジックバイト）、投稿への添付、拡大表示
  - [ ] 自動テスト

> **`/`（タイムライン）は実装済みです。** 無限スクロール・投稿作成（モーダル、**画像添付つき**）・編集・削除・
> 新着通知バナー・いいねが動きます。**投稿詳細にコメント欄も実装済み**（投稿・表示・削除）。
> **プロフィール表示・編集（プロフィール画像の変更・削除つき）、フォロー・フォロー中タイムラインも実装済み**です。
> **画像アップロードは AWS S3 とローカルディレクトリの2通りに対応**しており、`APP_STORAGE_TYPE` で切り替えます。
...
```
1. DB構築（Flywayマイグレーション）        ← 完了
2. 認証（JWT取得 → /auth/me が通る）       ← 完了
3. 投稿作成・全体TL                        ← 完了
4. フロント: ログイン + タイムライン表示    ← 完了
5. いいね ← カウンタの整合性をここで作り込む     ← 完了
6. コメント（カウンタの非対称ルール）        ← 完了。編集はPhase2
7. 画像アップロード（ストレージ抽象化を最初から入れる）← 完了。投稿添付・プロフィール画像も完了
8. フォロー + フォロー中TL ← MVPの一周が完成      ← 完了
9. プロフィール                             ← 完了
10. 削除と論理削除の徹底                    ← 完了
11. 無限スクロール                          ← 完了
```
...
> **`backend/` / `frontend/` / `docker-compose.yml` はすべて実装済みで、手順3〜5がそのまま動きます。**
> ブラウザでは**認証（新規登録 / ログイン / ログアウト）、タイムライン（投稿の作成・表示・編集・削除・いいね・**画像添付**）、
> 投稿詳細のコメント（投稿・表示・削除）、プロフィール（表示・編集・**プロフィール画像**）**を確認できます。
...
| フェーズ | 内容 |
|---|---|
| **MVP** | 27機能。「登録 → 投稿（画像添付） → いいね → コメント → フォロー → フォロー中TL」の一周。**すべて完了** |
| **Phase2** | コメントの編集、ユーザー検索、いいねしたユーザー一覧 |
