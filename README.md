...
- [ ] **実装フェーズ** — 進行中
  ...
  - [x] **プロフィール** — 表示（自分／他人）、編集（表示名・自己紹介）
  - [ ] 画像投稿・プロフィール画像
  - [ ] 自動テスト

> **`/`（タイムライン）は実装済みです。** 無限スクロール・投稿作成（モーダル）・編集・削除・
> 新着通知バナー・いいねが動きます。**投稿詳細にコメント欄も実装済み**（投稿・表示・削除）。
> **プロフィール表示・編集、フォロー・フォロー中タイムラインも実装済み**です。
> 画像添付・プロフィール画像は未実装です。
...
7. 画像アップロード（ストレージ抽象化を最初から入れる）← 次はここ
...
> ブラウザでは**認証（新規登録 / ログイン / ログアウト）、タイムライン（投稿の作成・表示・編集・削除・いいね）、
> 投稿詳細のコメント（投稿・表示・削除）**を確認できます。画像投稿はまだ動きません。
