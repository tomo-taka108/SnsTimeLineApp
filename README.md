...
- [ ] **実装フェーズ** — 進行中
  - [x] DB構築（`V1__create_users.sql` / `V2__create_refresh_tokens.sql` / `V3__create_posts_and_likes.sql` / `V4__create_follows.sql`）
  ...
  - [x] **投稿作成・全体タイムライン** — 作成 / 詳細 / 編集 / 削除、無限スクロール、新着通知バナー
  - [ ] いいね・コメント・画像投稿・フォロー登録
  - [ ] 自動テスト

> **`/`（タイムライン）は実装済みです。** 無限スクロール・投稿作成（モーダル）・編集・削除・
> 新着通知バナーが動きます。**いいねボタンは表示のみ**（`likes` テーブルは作成済み、APIは次のステップ）。
> コメント欄・画像添付は未実装です。
...
> ブラウザでは**認証（新規登録 / ログイン / ログアウト）とタイムライン（投稿の作成・表示・編集・削除）**
> を確認できます。いいね・コメント・画像投稿はまだ動きません。
